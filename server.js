...
app.use((req, res, next) => {
  ...
  if (bannedIPs.includes(ip)) return res.status(423).json({ banned: true, message: 'Ваш IP-адрес заблокирован' });
  next();
});
...
// IP-бан админские маршруты
app.post('/api/admin/ban-ip', auth, role('moderator'), (req, res) => {
  const user = users[req.body.username];
  if (!user?.lastIP) return res.status(400).json({ error: 'Нет IP' });
  if (bannedIPs.includes(user.lastIP)) return res.status(400).json({ error: 'Уже забанен' });
  bannedIPs.push(user.lastIP); save(path.join(DATA, 'banned_ips.json'), bannedIPs); res.json({ success: true });
});
app.post('/api/admin/unban-ip', auth, role('moderator'), (req, res) => {
  const idx = bannedIPs.indexOf(req.body.ip);
  if (idx === -1) return res.status(400).json({ error: 'Не найден' });
  bannedIPs.splice(idx, 1); save(path.join(DATA, 'banned_ips.json'), bannedIPs); res.json({ success: true });
});
app.get('/api/admin/banned-ips', auth, role('moderator'), (req, res) => res.json(bannedIPs));

// Остальные необходимые маршруты (админка пользователей, коды, статистика) могут быть добавлены из предыдущего полного server.js, в компактной версии опущены для краткости, но они нужны для полноценной работы. В реальном проекте вставьте их.
// ...
