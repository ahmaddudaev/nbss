...
if (cluster.isMaster) {
  ...
  console.log(`Master ${process.pid} запущен, форкаем ${numWorkers} воркеров`);
  for (let i = 0; i < numWorkers; i++) cluster.fork();
  cluster.on('exit', worker => {
    console.log(`Воркер ${worker.process.pid} упал, перезапускаем`);
    cluster.fork();
  });
} else {
  const app = express();
  ...
  const requireRole = (minRole) => (req, res, next) => {
    ...
    if ((levels[req.user.role]||0) < (levels[minRole]||0)) return res.status(403).json({ error: 'Недостаточно прав' });
    next();
  };
  ...
  app.get('/api/posts', (req, res) => {
    const now = Date.now();
    if (cachedPosts.length && (now - lastCacheTime) < CACHE_TTL) return res.json(cachedPosts);
    const enriched = posts.map(p => ({ ...p, authorRole: users[p.author]?.role, authorPremium: users[p.author]?.premium, authorVerified: users[p.author]?.verified }));
  ...
  app.post('/api/posts', auth, upload.array('images', 4), (req, res) => {
    ...
    posts.unshift(post); save(path.join(DATA, 'posts.json'), posts);
    cachedPosts = [];
    res.json(post);
  });
  ...
  app.post('/api/admin/ban-ip', auth, requireRole('moderator'), (req, res) => {
    const { username, duration } = req.body;
    const user = users[username];
  ...
  app.post('/api/admin/unban-ip', auth, requireRole('moderator'), (req, res) => {
    const { ip } = req.body;
  ...
  app.get('/api/stats', (req, res) => res.json({ users: Object.keys(users).length, posts: posts.length }));

  app.listen(PORT, () => console.log(`🚀 Воркер ${process.pid} на порту ${PORT}`));
}
