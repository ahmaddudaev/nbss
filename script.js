...
function renderComment(c) {
  ...
  return `
    <div class="comment">
      <div class="avatar-small">${c.author ? c.author[0]?.toUpperCase() : '?'}</div>
      <div class="comment-body">
        <span class="username ${nickClass}">${c.author || 'Аноним'}${verifiedIcon}</span>
...
async function loadProfile() {
  ...
  document.getElementById('profileStatus').innerHTML =
    (currentUser.verified ? '✅ Верифицирован' : '') +
    (currentUser.premium ? ' 💎 НБСС+' : '');
  try {
    ...
    container.innerHTML = userPosts.length
      ...
      : '<p style="padding:20px;">Нет постов</p>';
    attachPostActions();
  } catch (e) {
    console.error(e);
  }
