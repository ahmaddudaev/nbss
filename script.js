...
// Публикация с поддержкой нескольких изображений
...
if (postImageInput) {
  postImageInput.addEventListener('change', () => {
    selectedFiles = Array.from(postImageInput.files);
    renderImagePreviews();
  });
}
...
function renderImagePreviews() {
  ...
  selectedFiles.forEach((file, index) => {
    ...
    reader.onload = (e) => {
      ...
      removeBtn.className = 'btn outline remove-preview-btn';
...
async function loadMyProfile() {
  ...
  if (profilePosts) profilePosts.innerHTML = userPosts.length ? userPosts.map(p => renderPost(p)).join('') : '<p>Нет постов</p>';
  attachPostActions();
}
...
async function loadUserProfile(username) {
  try {
    ...
    const header = document.getElementById('profileHeader');
    if (header) header.innerHTML = '<p>Пользователь не найден</p>';
  }
}
...
async function loadDialogs() {
  ...
  try {
    ...
    document.querySelectorAll('.dialog-item').forEach(item => {
      item.addEventListener('click', () => { currentDialog = item.dataset.username; openChat(currentDialog); });
    });
...
async function openChat(username) {
  ...
  const partner = document.getElementById('chatPartner');
  if (partner) partner.textContent = username;
...
async function loadEvents() {
  ...
  try {
    ...
    document.querySelectorAll('.delete-event-btn').forEach(btn => {
      btn.addEventListener('click', async () => { if (confirm('Удалить ивент?')) { await request(`/events/${btn.dataset.eventId}`, { method: 'DELETE' }); loadEvents(); } });
    });
  } catch (e) {}
}
...
async function loadAdminUsers() {
  ...
  try {
    ...
    if (verifyBtn) verifyBtn.onclick = () => { if (!isModeratorOrAbove() && getSelected() !== currentUser.username) return alert('Нет прав'); if (!canModify() && getSelected() !== currentUser.username) return alert('Нельзя изменить этого пользователя'); modifyUser(getSelected(), { verified: true }); };
    if (unverifyBtn) unverifyBtn.onclick = () => { if (!isModeratorOrAbove() && getSelected() !== currentUser.username) return alert('Нет прав'); if (!canModify() && getSelected() !== currentUser.username) return alert('Нельзя изменить этого пользователя'); modifyUser(getSelected(), { verified: false }); };
    if (givePremiumBtn) givePremiumBtn.onclick = () => { if (!isModeratorOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя изменить этого пользователя'); modifyUser(getSelected(), { premium: true }); };
    if (revokePremiumBtn) revokePremiumBtn.onclick = () => { if (!isModeratorOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя изменить этого пользователя'); modifyUser(getSelected(), { premium: false }); };
    if (setOwnerBtn) setOwnerBtn.onclick = () => { if (!isOwner()) return alert('Только владелец может назначать владельцев'); if (!canModify()) return alert('Нельзя изменить роль этого пользователя'); modifyUser(getSelected(), { role: 'owner' }); };
    if (setHeadAdminBtn) setHeadAdminBtn.onclick = () => { if (!isOwner()) return alert('Только владелец может назначать главных администраторов'); if (!canModify()) return alert('Нельзя изменить роль этого пользователя'); modifyUser(getSelected(), { role: 'head_admin' }); };
    if (setAdminBtn) setAdminBtn.onclick = () => { if (!isHeadAdminOrAbove()) return alert('Только владелец или главный администратор могут назначать администраторов'); if (!canModify()) return alert('Нельзя изменить роль этого пользователя'); modifyUser(getSelected(), { role: 'admin' }); };
    if (setModeratorBtn) setModeratorBtn.onclick = () => { if (!isAdminOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя изменить роль этого пользователя'); modifyUser(getSelected(), { role: 'moderator' }); };
    if (setEventModeratorBtn) setEventModeratorBtn.onclick = () => { if (!isAdminOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя изменить роль этого пользователя'); modifyUser(getSelected(), { role: 'event_moderator' }); };
    if (removeRoleBtn) removeRoleBtn.onclick = () => { if (!isModeratorOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя изменить роль этого пользователя'); modifyUser(getSelected(), { role: 'user' }); };
    if (banUserBtn) banUserBtn.onclick = () => { if (!isModeratorOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя забанить этого пользователя'); modifyUser(getSelected(), { banUntil: new Date(Date.now() + 3600000).toISOString() }); };
    if (unbanUserBtn) unbanUserBtn.onclick = () => { if (!isModeratorOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя разбанить этого пользователя'); modifyUser(getSelected(), { banUntil: null }); };
    if (deleteUserBtn) deleteUserBtn.onclick = () => { if (!isModeratorOrAbove()) return alert('Нет прав'); if (!canModify()) return alert('Нельзя удалить этого пользователя'); if (confirm(`Удалить ${getSelected()}?`)) modifyUser(getSelected(), { delete: true }); };
  } catch (e) { select.innerHTML = '<option>Ошибка загрузки</option>'; }
}
async function modifyUser(username, changes) {
  try { await request(`/admin/user/${username}`, { method: 'POST', body: JSON.stringify(changes) }); loadAdminUsers(); } catch (e) { alert('Ошибка: ' + e.message); }
}
